...
bashrc = Path("bashrc")
security_test = Path("tests/test-security-boundaries.sh")
text = bashrc.read_text(encoding="utf-8")
test = security_test.read_text(encoding="utf-8")

helper_start = text.index("_aur_guard_scan_checkout_with_aur_scan() {")
helper_end = text.index("\n_aur_guard_normalize_srcinfo() {", helper_start)
old_helper = text[helper_start:helper_end]
if "--fail-on high" not in old_helper:
    raise SystemExit("old aur-scan severity gate is not present exactly where expected")

new_helper = r'''declare -a _AUR_GUARD_SCAN_REVIEW_FINDINGS=()
...
reset_old = "  _AUR_GUARD_HISTORICAL_MATCHES=()\n  _AUR_GUARD_CONTEXT_WARNINGS=()\n  _AUR_GUARD_INSTALL_STARTED=0\n"
reset_new = "  _AUR_GUARD_HISTORICAL_MATCHES=()\n  _AUR_GUARD_CONTEXT_WARNINGS=()\n  _AUR_GUARD_SCAN_REVIEW_FINDINGS=()\n  _AUR_GUARD_INSTALL_STARTED=0\n"
if text.count(reset_old) != 1:
    raise SystemExit(f"expected one verification reset block, found {text.count(reset_old)}")
...
status_start = verify.index("  if (( status == 0 )); then\n")
cleanup_start = verify.index("\n  _aur_guard_cleanup_work\n", status_start)
new_status = '''  if (( status == 0 )); then
...
'''
verify = verify[:status_start] + new_status + verify[cleanup_start:]
text = text[:verify_start] + verify + text[verify_end:]

install_start = text.index("aurinstall() {")
install_end = text.index("\n# Backward-compatible AUR Guard install command.", install_start)
install = text[install_start:install_end]
if '_aur_guard_confirm_scan_review_install "$pkg"' in install:
    raise SystemExit("scanner review install boundary already exists")
identity_start = install.index("    if ! _aur_guard_confirm_identity_changes; then\n")
build_start = install.index('    if ! _aur_guard_build_verified_artifacts "$pkg"; then\n', identity_start)
review_boundary = '''    if ! _aur_guard_confirm_scan_review_install "$pkg"; then
...
'''
install = install[:build_start] + review_boundary + install[build_start:]
text = text[:install_start] + install + text[install_end:]

old_comment = "# PKGBUILD. High and critical aur-scan findings are a hard failure. The stable\n# aur-scanner package is the automatic bootstrap target when the binary is absent.\n"
new_comment = "# PKGBUILD. Scanner findings remain visible evidence; scanner execution or malformed\n# output fails closed. The stable aur-scanner package is the automatic bootstrap target.\n"
if test.count(old_comment) != 1:
    raise SystemExit("old scanner policy comment was not found exactly once")
test = test.replace(old_comment, new_comment, 1)

old_command = 'assert_contains "$BASHRC" \'"$scanner" scan "$pkgdir" --fail-on high\'\n'
new_command = 'assert_contains "$BASHRC" \'"$scanner" scan "$pkgdir" --format json\'\nassert_not_contains "$BASHRC" \'--fail-on high\'\n'
if test.count(old_command) != 1:
    raise SystemExit("old scanner command assertion was not found exactly once")
test = test.replace(old_command, new_command, 1)

fake_start = test.index('cat >"${aur_scan_fakebin}/aur-scan" <<\'EOF_AUR_SCAN\'\n')
fake_end = test.index("EOF_AUR_SCAN\n", fake_start) + len("EOF_AUR_SCAN\n")
fake_new = '''cat >"${aur_scan_fakebin}/aur-scan" <<'EOF_AUR_SCAN'
#!/usr/bin/env bash
set -euo pipefail
...
EOF_AUR_SCAN
'''
test = test[:fake_start] + fake_new + test[fake_end:]

args_start = test.index('[[ ${aur_scan_args[2]} == --fail-on && ${aur_scan_args[3]} == high ]]')
args_end = test.index("\n\nif env", args_start)
args_new = '''[[ ${aur_scan_args[2]} == --format && ${aur_scan_args[3]} == json ]] \\
  || fail 'AUR Guard did not request structured aur-scan JSON output'
'''
test = test[:args_start] + args_new + test[args_end:]

bashrc.write_text(text, encoding="utf-8")
security_test.write_text(test, encoding="utf-8")
