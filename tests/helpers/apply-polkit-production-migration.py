...
SERVICE_PATH = ROOT / "config/hypr/scripts/awtarchy-polkit-agent/awtarchy-polkit-agent.service"
...
update_old = '''  apply_plan "$plan_file" || die "Update failed and user files were rolled back."

  hardware_reconcile
'''
update_new = '''  apply_plan "$plan_file" || die "Update failed and user files were rolled back."
  ...
  hardware_reconcile
'''
runtime = replace_once(runtime, update_old, update_new, "update integration")
...
hypr = replace_once(
    ...
    "Hyprland PolicyKit autostart",
)
HYPR_PATH.write_text(hypr, encoding="utf-8")

service = SERVICE_PATH.read_text(encoding="utf-8")
service = replace_once(service, "\n[Install]\nWantedBy=default.target\n", "\n", "service default-target install section")
SERVICE_PATH.write_text(service, encoding="utf-8")

launcher = LAUNCHER_PATH.read_text(encoding="utf-8")
old_parser = "    read -r uid mode type < <(/usr/bin/stat -Lc '%u %a %F' -- \"$path\") || return 1"
new_parser = "    IFS=' ' read -r uid mode type < <(/usr/bin/stat -Lc '%u %a %F' -- \"$path\") || return 1"
if launcher.count(old_parser) != 2:
    raise SystemExit(f"launcher stat parser: expected 2 matches, found {launcher.count(old_parser)}")
launcher = launcher.replace(old_parser, new_parser)
