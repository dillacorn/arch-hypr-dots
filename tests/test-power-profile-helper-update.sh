...
require_literal "$RECONCILER" 'POWER_PROFILE_HELPER_SOURCE=' \
  'power reconciler has no trusted helper source'
...
require_literal "$RECONCILER" "[[ \$(head -n1 -- \"\$POWER_PROFILE_HELPER_SOURCE\") == '#!/usr/bin/bash' ]]" \
  'power reconciler does not validate the helper interpreter'
