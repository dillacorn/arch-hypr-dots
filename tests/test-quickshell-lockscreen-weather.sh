...
' "$CACHE" >/dev/null || fail 'weather helper wrote an invalid cache payload'

grep -Fq -- 'geocoding-api.open-meteo.com' "$TEST_CURL_LOG" \
    || fail 'weather helper did not use Open-Meteo geocoding'
...
if PATH="$TMP/bin:$PATH" XDG_CACHE_HOME="$TMP/cache" HOME="$TMP/home" \
    ...
    fail 'weather helper accepted an empty location'
fi
after_calls="$(wc -l <"$TEST_CURL_LOG")"
[[ "$before_calls" -eq "$after_calls" ]] \
    || fail 'empty weather location still triggered a network request'
...
chmod 0755 "$TMP/bin/curl"
if PATH="$TMP/bin:$PATH" XDG_CACHE_HOME="$TMP/cache" HOME="$TMP/home" \
    bash "$HELPER" refresh 'Pittsburgh, PA' >/dev/null 2>&1; then
    fail 'weather helper reported success after a network failure'
fi
cmp -s "$CACHE" "$TMP/cache-before.json" \
    || fail 'failed weather refresh destroyed or replaced the prior cache'
