...
grep -Fq -- "$move_guard" "$SHELL_QML" \
  || fail 'shell does not detect Hyprland workspace-move events that invalidate Quickshell monitor state'
grep -Fq -- "$refresh_call" "$SHELL_QML" \
  || fail 'shell does not refresh authoritative Hyprland monitor state after a workspace move'

python3 - "$SHELL_QML" <<'PY'
from pathlib import Path
import sys

text = Path(sys.argv[1]).read_text(encoding="utf-8")
guard = 'if (event.name === "moveworkspace" || event.name === "moveworkspacev2") {'
start = text.find(guard)
if start < 0:
    raise SystemExit("workspace-move guard missing")
end = text.find("}", start)
if end < 0:
    raise SystemExit("workspace-move guard is unterminated")
block = text[start:end + 1]
if "Hyprland.refreshMonitors();" not in block:
    raise SystemExit("workspace-move guard does not refresh monitor state")
PY
...
grep -Fq '.config/quickshell/awtarchy/shell.qml' "$RUNTIME" \
  || fail 'v3.4.2 workspace-focus repair does not target shell.qml'
...
v342_home="${TMP}/v342"
v341_home="${TMP}/v341"
for home in "$v342_home" "$v341_home"; do
  mkdir -p "${home}/.config/quickshell/awtarchy"
  printf '%s\n%s\n' "$active_expr" "$active_expr" \
    >"${home}/.config/quickshell/awtarchy/Bar.qml"
  cat >"${home}/.config/quickshell/awtarchy/shell.qml" <<'EOF_SHELL'
    Connections {
        target: Hyprland
...
v342_shell="${v342_home}/.config/quickshell/awtarchy/shell.qml"
...
grep -Fq -- "$move_guard" "$v342_shell" \
  || fail 'real v3.4.2 repair did not add the workspace-move monitor refresh guard'
grep -Fq -- "$refresh_call" "$v342_shell" \
  || fail 'real v3.4.2 repair did not refresh monitor state after workspace moves'

repair_v342_workspace_focus_target "$v342_home" v3.4.2
[[ $(grep -Fc -- "$focused_expr" "$v342_bar") -eq 2 ]] \
  || fail 'v3.4.2 workspace-focus Bar repair is not idempotent'
[[ $(grep -Fc -- "$move_guard" "$v342_shell") -eq 1 ]] \
  || fail 'v3.4.2 workspace-focus shell repair is not idempotent'
[[ $(grep -Fc -- "$refresh_call" "$v342_shell") -eq 1 ]] \
  || fail 'v3.4.2 workspace-focus repair duplicated monitor refresh calls'

repair_v342_workspace_focus_target "$v341_home" v3.4.1
v341_bar="${v341_home}/.config/quickshell/awtarchy/Bar.qml"
v341_shell="${v341_home}/.config/quickshell/awtarchy/shell.qml"
[[ $(grep -Fc -- "$active_expr" "$v341_bar") -eq 2 ]] \
  || fail 'workspace-focus post-release repair changed a non-v3.4.2 Bar target'
if grep -Fq -- "$focused_expr" "$v341_bar"; then
  fail 'workspace-focus post-release Bar repair leaked into another release tag'
fi
if grep -Fq -- "$move_guard" "$v341_shell" || grep -Fq -- "$refresh_call" "$v341_shell"; then
  fail 'workspace-focus post-release shell repair leaked into another release tag'
fi
