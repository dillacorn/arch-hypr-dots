...
installer = replace_once(
    installer,
    marker,
    refresh_function + marker,
    "existing-install message marker",
)
...
installer, replaced = message_pattern.subn(message_replacement, installer, count=1)
if replaced != 1:
    raise SystemExit(f"expected one existing-install message function, replaced {replaced}")
...
new_existing = dedent(
    '''\
    cat >"$home/.local/bin/awtarchy" <<'EOF_STALE_LAUNCHER'
    #!/usr/bin/env bash
    exit 99
    EOF_STALE_LAUNCHER
...
lua_test_path.write_text(lua_test, encoding="utf-8")
